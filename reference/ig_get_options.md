...
A tibble with options/derivative position details, including columns
like \`dealId\`, \`size\`, \`direction\`, \`instrumentType\`, and others
as returned by the IG API \`/positions\` endpoint. If \`instrumentType\`
is available, filters for "OPTION", "DERIVATIVE", or "OPTION_CONTRACT";
