...
test_that("ig_search_markets returns tibble from mock_response", {
  ...
  res <- ig_search_markets("test", auth = list(base_url = "https://demo-api.ig.com", cst = "x", security = "y"), mock_response = mock)
...
test_that("ig_get_price handles mock_response and returns tibble", {
  ...
  res <- ig_get_price("TEST.EPIC", auth = list(base_url = "https://demo-api.ig.com", cst = "x", security = "y"), mock_response = mock)
  expect_s3_class(res, "tbl_df")
...
test_that("ig_get_accounts returns tibble from mock_response", {
  ...
  res <- ig_get_accounts(auth = list(base_url = "https://demo-api.ig.com", cst = "x", security = "y"), mock_response = mock)
  expect_s3_class(res, "tbl_df")
  expect_true("accountId" %in% names(res))
})

test_that("ig_logout returns TRUE in testing mode", {
  Sys.setenv(IGFETCHR_TESTING = "true")
  on.exit(Sys.unsetenv("IGFETCHR_TESTING"))
  res <- ig_logout(list(base_url = "https://demo-api.ig.com", cst = "x", security = "y"))
  expect_true(isTRUE(res) || identical(res, invisible(TRUE)))
})
